...
- **Husky pre-commit hook runs automatically** (configured in `.husky/pre-commit`):
  - Prettier formats staged TS/JS/JSON/MD files
  - ESLint checks and auto-fixes staged TS/JS files
  - Only processes **staged files**, not entire codebase
- Skip hooks only if absolutely necessary: `git commit -n` or `git commit --no-verify`
- If you see git stash errors, ensure all changes are staged or stashed first
- See `.github/GIT_HOOKS.md` for troubleshooting
