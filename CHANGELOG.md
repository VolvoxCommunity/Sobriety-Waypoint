...
- Strengthen type definitions by replacing `any` with proper types: add `NotificationData` union type for notification payloads, add `SanitizedValue` type for Sentry privacy utilities, use `keyof SupabaseClient` for Supabase proxy
